...
void _USAO_CB_EX(int parent_id) {
	// Nothing.
}
...
UnitSAO::UnitSAO(ServerEnvironment *env, v3f pos) : ServerActiveObject(env, pos)
{
	...
	onAttach = &_USAO_CB_EX;
	onDetach = &_USAO_CB_EX;
}
...
void UnitSAO::sendOutdatedData()
{
	if (!m_armor_groups_sent) {
		...
		m_messages_out.push(ActiveObjectMessage(getId(), true, generateUpdateArmorGroupsCommand()));
	}
...
void UnitSAO::sendPunchCommand()
{
	...
	m_messages_out.push(ActiveObjectMessage(getId(), true, generatePunchCommand(result_hp),
