...
radius, std::vector<ServerActiveObject *> &result, std::function<bool(ServerActiveObject *obj)>
include_obj_cb, uint16_t mapid) {
...
void ActiveObjectMgr::getObjectsInArea(const aabb3f &box, std::vector<ServerActiveObject *>
&result, std::function<bool(ServerActiveObject *obj)> include_obj_cb, uint16_t mapid)
{
	for (auto &activeObject : m_active_objects) {
		ServerActiveObject *obj = activeObject.second;
...
void ActiveObjectMgr::getAddedActiveObjectsAroundPos(const v3f &player_pos, f32 radius,
		...
		std::queue<u16> &added_objects, uint16_t mapid)
{
	/*
	...
	*/
	for (auto &ao_it : m_active_objects) {
		...
		// Check if object are in the same environment as the player
