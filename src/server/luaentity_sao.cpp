...
// WARNING: LuaEntitySAO will remain as it is. But will handle callbacks from addons.

void _c0(uint16_t a, float b, collisionMoveResult*c) {}
void _c1(uint16_t a) {}
bool _c2(uint16_t a, const char *b) {return false;}
void _c3(uint16_t a, LuaEntitySAO *b, ObjectProperties *c) {}
void _c4(uint16_t a, std::string b, float c) {}
std::string _c5(uint16_t a) {return std::string();}
bool _c6(uint16_t a, ServerActiveObject *b, float c, const ToolCapabilities *d, v3f e, uint16_t f) {return false;}
bool _c7(uint16_t a, ServerActiveObject *b) {return false;}

LuaEntitySAO::LuaEntitySAO(ServerEnvironment *env, v3f pos, const std::string &data, uint16_t
mapid)
	: UnitSAO(env, pos)
{
	builtin_step_ = &_c0;
	builtin_delete = &_c1;
	builtin_spawned = &_c2;
	builtin_getProperties = &_c3;
	builtin_activate = &_c4;
	builtin_deactivate = &_c1;
	builtin_getStaticData = &_c5;
	builtin_punch = &_c6;
	builtin_ondeath = &_c7;
	builtin_onrightclick = &_c7;
	std::string name;
	std::string state;
	u16 hp = 1;
	v3f velocity;
...
void LuaEntitySAO::step(float dtime, bool send_recommended)
{
	...
	if(m_registered) {
		...
		builtin_step_(m_id, dtime, moveresult_p);
	}
...
std::string LuaEntitySAO::getClientInitializationData(u16 protocol_version, u16 used_id)
{
...
//std::string LuaEntitySAO::GetInitData(u16 proto)
//{
//	return std::string(""); //Should not be called on LuaEntitySao....
//}

void LuaEntitySAO::getStaticData(std::string *result)
{
	...
	if(m_registered){
		...
		std::string state = builtin_getStaticData(m_id);
		...
		float time_from_last_punch)
{
	...
	//bool damage_handled = m_env->getScriptIface()->luaentity_Punch(m_id, puncher, time_from_last_punch, toolcap, dir, result.did_punch ? result.damage : 0);
	bool damage_handled = builtin_punch(m_id, puncher, time_from_last_punch, toolcap, dir, result.did_punch ? result.damage : 0);
	...
	if (getHP() == 0 && !isGone()) { // NEW: on_death should return true IF the entity should continue existing.
		//m_env->getScriptIface()->luaentity_on_death(m_id, puncher);
		if (builtin_ondeath(m_id, puncher))
			goto CONTINUE;
		clearParentAttachment();
...
void LuaEntitySAO::rightClick(ServerActiveObject *clicker)
{
	...
	builtin_onrightclick(m_id, clicker);
...
void LuaEntitySAO::setTextureMod(const std::string &mod)
{
	...
	// create message and add to list
	m_messages_out.push(ActiveObjectMessage(getId(), true, generateSetTextureModCommand()));
}
		...
		bool select_horiz_by_yawpitch)
{
	...
	// create message and add to list
	m_messages_out.push(ActiveObjectMessage(getId(), true, str));
}
