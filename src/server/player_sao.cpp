...
PlayerSAO::PlayerSAO(ServerEnvironment *env_, RemotePlayer *player_, maskedu16 peer_id_, bool is_singleplayer):
	UnitSAO(env_, v3f(0,0,0)),
	m_player(player_),
	player_id(peer_id_.value),
	m_is_singleplayer(is_singleplayer)
{
	SANITY_CHECK(player_id != 0);
...
std::string PlayerSAO::getClientInitializationData(u16 protocol_version, u16 ID)
{
...
std::string PlayerSAO::GetInitData(u16 proto)
{
	std::ostringstream os(std::ios::binary);
	writeV3F(os, m_base_position, 37);
	writeV3F32(os, m_rotation);
	writeF32(os, m_pitch);
	writeU16(os, m_hp);
	std::ostringstream msg_os(std::ios::binary);
	//Some messages in the internal system, which will be sent ONLY to 1 player
	msg_os << serializeString32(getPropertyPacket(proto));
	msg_os << serializeString32(generateUpdateArmorGroupsCommand());
	msg_os << serializeString32(generateUpdateAnimationCommand(proto));
	msg_os << serializeString32(generateUpdateAttachmentCommand(proto));
	//Misc messages
	u8 msg = 0;
	for (const auto &bone_pos : m_bone_position) {
		msg_os << serializeString32(generateUpdateBonePositionCommand(bone_pos.first, bone_pos.second.X, bone_pos.second.Y, proto));
		msg++;
	}
	for (const auto &id : getAttachmentChildIds()) {
		if (ServerActiveObject *obj = m_env->getActiveObject(id)) {
			msg++;
			msg_os << serializeString32(obj->generateUpdateInfantCommand(id, proto));
		}
	}
	writeU8(os, msg);
	std::string serialized = msg_os.str();
...
void PlayerSAO::step(float dtime, bool send_recommended)
{
	...
	if (m_player->inventory.checkModified())
		m_env->m_server->SendInventory(this, true); //This should've on the player_sao part

	//BEETHOVEN IS FIREEEEEEEEE
	RemotePlayer *player = getPlayer();
	if (!player->multicraftplayer) {
		if (player->sneakpressed != player->lastsneakpressed) {
			player->lastsneakpressed = player->sneakpressed;
			//means true ...
			if (player->sneakpressed) {
				m_prop.eye_height = 1.45f;
	...
	if (!m_properties_sent) {
		...
		m_messages_out.push(ActiveObjectMessage(getId(), true, str, legacy_str));
		m_env->queuePlayerEvent(this);
		//m_env->getScriptIface()->player_event(this, "properties_changed"); //Must be queued to a global lua state
	}
	...
	if (m_position_not_sent) {
		...
		// create message and add to list
		m_messages_out.push(ActiveObjectMessage(getId(), false, str, legacy_str));
	}
...
//__CONTINUE:
	...
	if (!m_physics_override_sent) {
		...
		// create message and add to list
		m_messages_out.push(ActiveObjectMessage(getId(), true,
...
void PlayerSAO::setBasePosition(const v3f &position, bool dont_send)
{
...
void PlayerSAO::setPos(const v3f &pos)
{
	...
	m_last_good_position = pos;
	m_move_pool.empty();
	m_time_from_last_teleport = 0.0f;
	...
	float time_from_last_punch)
{
	...
	m_env->queueOnPunchPlayer(playersao, puncher, time_from_last_punch, toolcap, dir, hitparams.hp); //getScriptIface()->on_punchplayer();
	bool damage_handled = false; //FIXME: CALLBACK ON PUNCH PLAYER
	if (!damage_handled) {
...
{
	m_env->queueOnRightClickPlayer(this, clicker);
}
...
void PlayerSAO::setHP(s32 hp, const PlayerHPChangeReason &reason)
{
	...
	{
		m_env->queueOnChangeHP(this, m_hp - hp, reason); //NOTE: Why it is reversed? //getScriptIface()->on_player_hpchange(this, hp - m_hp, reason);
		s32 hp_change = 0; //FIXME: CHANGE THIS TO BE AN CALLBACK > PLEASE
		if (hp_change == 0)
			return;
...
void PlayerSAO::disconnected(bool goingto_otherserv)
{
	if (!goingto_otherserv)
...
{
	//If this is an 'slave' server then delete the object completely
	...
	//If moving to other server then we should NOT remove the RemotePlayer* as it are useful to redirect packets
	if (!m_player->to_other_server) {
		assert(m_player->getPlayerSAO() == this);
