...
void printNdebug(std::string modorig, std::string name, bool legacy) {
#ifdef NDEBUG
	//nt
#endif
}
		...
		u32 active_object_count, u32 active_object_count_wider)
{
	...
	//std::cout << std::this_thread::get_id() << std::endl; //debug from abm_threading.cpp
	...
	//Should we check which type should call [legacy_abm or abmT/E]
	lua_getfield(L, -1, "legacy_abm");
	bool is_legacy = lua_toboolean(L, -1);
	lua_remove(L, -1);

	lua_getfield(L, -1, "mod_origin");
	const char *mod_orig = luaL_checkstring(L, -1);
	lua_remove(L, -1);
	...
	// Call action
	//printNdebug(mod_orig, label, is_legacy);
	if (is_legacy) {
		luaL_checktype(L, -1, LUA_TTABLE);
		...
		lua_remove(L, -2);
		//serialize
		call_string_dump(L, -2);
		size_t func_length;
		const char *funcsrz = lua_tolstring(L, -1, &func_length);
		scriptIface->queueABM(std::string(funcsrz, func_length), p, n, active_object_count,
...
int ModApiEnvMod::l_set_node(lua_State *L)
{
	...
	// Do it
	bool succeeded = env->setNode(pos, n, mapid);
	lua_pushboolean(L, succeeded);
	return 1;
}

// bulk_set_node([pos1, pos2, ...], node, mapid)
// pos = {x=num, y=num, z=num}
// mapid = num
// node = {node info}
int ModApiEnvMod::l_bulk_set_node(lua_State *L)
{
	...
	for (s32 i = 1; i <= len; i++) {
		lua_rawgeti(L, 1, i);
		if (!env->setNode(read_v3s16(L, -1), n, mapid))
			succeeded = false;
		lua_pop(L, 1);
	}
...
int ModApiEnvMod::l_remove_node(lua_State *L)
{
	...
	uint16_t mapid = 0;
	if (lua_isnumber(L, 2)) {
		mapid = lua_tonumber(L, 2);
	}
	// Do it
	bool succeeded = env->removeNode(pos, mapid);
	lua_pushboolean(L, succeeded);
	return 1;
}
...
int ModApiEnvMod::l_swap_node(lua_State *L)
{
	...
	// Do it
	bool succeeded = env->swapNode(pos, n, mapid);
	lua_pushboolean(L, succeeded);
	return 1;
}
...
int ModApiEnvMod::l_get_node(lua_State *L)
{
	...
	// Do it
	MapNode n = env->getMap(mapid).getNode(pos);
	// Return node
	pushnode(L, n, env->getGameDef()->ndef());
	return 1;
}
...
int ModApiEnvMod::l_get_node_or_nil(lua_State *L)
{
	...
	// Do it
	bool pos_ok;
	MapNode n = env->getMap(mapid).getNode(pos, &pos_ok);
	if (pos_ok) {
		// Return node
		pushnode(L, n, env->getGameDef()->ndef());
	} else {
...
int ModApiEnvMod::l_get_natural_light(lua_State *L)
{
	...
	v3s16 pos = read_v3s16(L, 1);
	//Check if mapid available
	uint16_t map = 0;
	if (lua_isnumber(L, 2))
		map = lua_tonumber(L, 2);

	bool is_position_ok;
	MapNode n = env->getMap(map).getNode(pos, &is_position_ok);
	if (!is_position_ok)
		return 0;
	...
	try {
		if (daylight == n.param1 >> 4)
			daylight = env->findSunlight(pos, map);
...
int ModApiEnvMod::l_place_node(lua_State *L)
{
	...
	v3s16 pos = read_v3s16(L, 1);
	MapNode n = readnode(L, 2, ndef);
	//Get player, if specified; NOTE: Compatibility with newest mods
	const char *name = luaL_checkstring(L, 3);
	RemotePlayer *player = env->getPlayer(name);
	PlayerSAO *sao = nullptr;
	if (player && player->getPeerId() != PEER_ID_INEXISTENT) {
		sao = player->getPlayerSAO();
	}

	uint16_t map = 0;
	if (lua_isnumber(L, 4))
		map = lua_tonumber(L, 4);

	// Don't attempt to load non-loaded area as of now
	MapNode n_old = env->getMap(map).getNode(pos);
	...
	// Place it with a NULL placer (appears in Lua as nil) //FIXME: Nuh uh.
	bool success = scriptIfaceItem->item_OnPlace(item, sao, pointed, map);
...
int ModApiEnvMod::l_dig_node(lua_State *L)
{
	...
	// Don't attempt to load non-loaded area as of now
	MapNode n = env->getMap(map).getNode(pos);
	...
	bool success = scriptIfaceNode->node_on_dig(pos, n, sao, map);
	lua_pushboolean(L, success);
	return 1;
}
...
int ModApiEnvMod::l_punch_node(lua_State *L)
{
	...
	// ObjectRef)
	bool success = scriptIfaceNode->node_on_punch(pos, n, sao, PointedThing(), map);
	lua_pushboolean(L, success);
	return 1;
}
...
int ModApiEnvMod::l_add_entity(lua_State *L)
{
	...
	if(objectid == 0) {
		errorstream << "Failed to add entity " << name << " at " << PP(pos) << std::endl;
		return 0;
	}
...
int ModApiEnvMod::l_add_item(lua_State *L)
{
	...
	lua_pushvalue(L, mapid);

	PCALL_RESL(L, lua_pcall(L, 3, 1, error_handler));
...
int ModApiEnvMod::l_get_connected_players(lua_State *L)
{
	...
	if (!env) {
		log_deprecated(L, "Calling get_connected_players() at mod load time is deprecated");
	...
	for (RemotePlayer *player : env->getPlayers()) {
		if (player->getPeerId() == PEER_ID_INEXISTENT) // Update v4.0 [MineStars]: PeerID can be player id
			continue;
...
int ModApiEnvMod::l_get_objects_inside_radius(lua_State *L)
{
	...
	uint16_t mapid = 0;
	if (lua_isnumber(L, 3))
		mapid = lua_tonumber(L, 3); //Map id specified, or default: 0
	std::vector<ServerActiveObject *> objs;

	if (mapid != 0 && !getServer(L)->existsMap(mapid)) {
		errorstream << "Lua side: get_objects_in_area: Unknown map id: " << mapid << std::endl;
		lua_createtable(L, 0, 0);
		return 0;
	}

	auto include_obj_cb = [](ServerActiveObject *obj){ return !obj->isGone(); };
	env->getObjectsInsideRadius(objs, pos, radius, include_obj_cb, mapid);
...
int ModApiEnvMod::l_get_objects_in_area(lua_State *L)
{
	...
	auto include_obj_cb = [](ServerActiveObject *obj){ return !obj->isGone(); };
	env->getObjectsInArea(objs, box, include_obj_cb, mapid);
...
template <typename F>
int ModApiEnvMod::findNodesInAreaUnderAir(lua_State *L, v3s16 minp, v3s16 maxp,
...
template <typename F>
int ModApiEnvMod::findNodeNear(lua_State *L, v3s16 pos, int radius,
...
template <typename F>
int ModApiEnvMod::findNodesInArea(lua_State *L, const NodeDefManager *ndef,
...
void ModApiEnvMod::checkArea(v3s16 &minp, v3s16 &maxp)
{
...
int ModApiEnvMod::l_line_of_sight(lua_State *L)
{
	...
	bool success = env->line_of_sight(pos1, pos2, &p, mapid);
	lua_pushboolean(L, success);
	if (!success) {
		push_v3s16(L, p);
		return 2;
...
int ModApiEnvMod::l_fix_light(lua_State *L)
{
	...
	if (!getServer(L)->existsMap(mapid))
		return 0;

	ServerMap &map = env->getServerMap(mapid);
...
int ModApiEnvMod::l_emerge_area(lua_State *L)
{
	...
	EmergeManager *emerge = getServer(L)->getEmergeManager(mapid);
...
int ModApiEnvMod::l_find_path(lua_State *L)
{
	...
	uint16_t mapid = 0;
	if (lua_isnumber(L, 7)) {
		mapid = lua_tonumber(L, 7);
	}

	if (!getServer(L)->existsMap(mapid))
		return 0;

	std::vector<v3s16> path = get_path(&env->getServerMap(mapid), env->getGameDef()->ndef(), pos1, pos2,
...
int ModApiEnvMod::l_transforming_liquid_add(lua_State *L)
{
	...
	env->getMap(mapid).transforming_liquid_add(p0);
	return 1;
}
