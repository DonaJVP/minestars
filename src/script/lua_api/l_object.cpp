...
int ObjectRef::l_send_mapblock(lua_State *L)
{
	...
	uint16_t peer_id = player->getPeerId();
	r = getServer(L)->SendBlock(peer_id, pos);
...
int ObjectRef::l_add_velocity(lua_State *L)
{
	...
	if (sao->getType() == ACTIVEOBJECT_TYPE_LUAENTITY) {
		...
		srv->SendPlayerSpeed(playersao->getPeerID(), vel);
	}
...
//MINESTARS
int ObjectRef::l_set_multicraft_value(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	//lua_pushstring(L, player->getName());
	bool thing = readParam<bool>(L, 2, false);
	player->multicraftplayer = thing;
	return 1;
}

int ObjectRef::l_set_player_sum(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	float sumy = readParam<float>(L, 2, 0.0f);
	float sumz = readParam<float>(L, 3, 0.0f);
	//if (player->sumY != sumy)
		player->sumY = sumy;
	//if (player->sumZ != sumz)
		player->sumZ = sumz;
...
int ObjectRef::l_set_fov(lua_State *L)
{
	...
	Server *srv = getServer(L);
	srv->SendPlayerFov(player->getPeerId());
	return 0;
}
...
int ObjectRef::l_set_minimap_modes(lua_State *L)
{
	...
	srv->SendMinimapModes(player->getPeerId(), modes, selected_mode);
	return 0;
}
...
luaL_Reg ObjectRef::methods[] = {
	...
	luamethod(ObjectRef, set_multicraft_value),
	luamethod(ObjectRef, set_player_sum),
