...
int ModApiServer::l_register_map(lua_State *L) {
	NO_MAP_LOCK_REQUIRED;
	std::string name = readParam<std::string>(L, 1);
	uint16_t value = 0;
	bool res = getServer(L)->createNewMap(name, &value);
	lua_pushboolean(L, res);
	return 1;
}

int ModApiServer::l_put_player_to_map(lua_State *L) {
	GET_ENV_PTR;
	std::string name = readParam<std::string>(L, 1);
	u16 idfors = (u16)lua_tonumber(L, 2);
	RemotePlayer *player = env->getPlayer(name.c_str());
	getServer(L)->setPlayerOnMap(player, idfors);
	return 0;
}
...
void ModApiServer::Initialize(lua_State *L, int top)
{
	...
	API_FCT(register_map);
	API_FCT(put_player_to_map);
}
