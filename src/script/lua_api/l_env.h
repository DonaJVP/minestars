...
#include "abm_threading.h" // depend on ActiveBlockModifier [moved]
#include "raycast.h"
...
protected:
	...
	static int findNodeNear(lua_State *L, v3s16 pos, int radius,
		const std::vector<content_t> &filter, int start_radius, F &&getNode); //x
	...
	static int findNodesInArea(lua_State *L,  const NodeDefManager *ndef,
		const std::vector<content_t> &filter, bool grouped, F &&iterate);//x

	// F must be (v3s16 pos) -> MapNode
	template <typename F>
	static int findNodesInAreaUnderAir(lua_State *L, v3s16 minp, v3s16 maxp,
		const std::vector<content_t> &filter, F &&getNode); //x
public:
 ...
 */
class ModApiEnvVM : public ModApiEnvMod {
private:
