...
class PlayerSAO;
// Special functions.
using _callback_ACT0 = void(*)(v3s16, MapNode, uint16_t);
using _callback_ACT1 = void(*)(ItemStack&, PlayerSAO*, PointedThing);
using _callback_ACT1_B = bool(*)(ItemStack&, PlayerSAO*, PointedThing);
...
{
	// Some functions.
	_callback_ACT0 on_construct = nullptr;
	_callback_ACT0 on_destruct = nullptr;
	_callback_ACT0 after_destruct = nullptr;
	_callback_ACT1 on_place = on_place_root;
	_callback_ACT1_B on_dig = on_dig_root;
	_callback_ACT1 on_rightclick = nullptr;
	_callback_ACT1_B on_punch = nullptr;
...
public:
	...
	//c o p y e s  t h e  C o n t e n t F e a t u r e s
	const ContentFeatures copy(content_t c) {
	 ...
	 */
	content_t set(const std::string &name, const ContentFeatures &def, content_t forced_id = 0);
	...
	std::vector<ContentFeatures> getAll() { return m_content_features; }
	void getAll(std::vector<ContentFeatures> &result);

	//content_t getMax() { return }
	size_t getSize() { return m_content_features.size(); }
	
	void setMNEXT_ID(u32 new_raw_size);
private:
	/*!
