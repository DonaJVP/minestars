	...
	m_gamedef(gamedef)
{
	...
	//m_cache_abm_interval = g_settings->getFloat("abm_interval"); //On abm_threading.cpp|h
	m_cache_nodetimer_interval = g_settings->getFloat("nodetimer_interval");
	//m_cache_abm_time_budget = g_settings->getFloat("abm_time_budget"); //On abm_threading.cpp|h
...
bool Environment::line_of_sight(v3f pos1, v3f pos2, v3s16 *p, uint16_t map)
{
	// Iterate trough nodes on the line
	voxalgo::VoxelLineIterator iterator(pos1 / BS, (pos2 - pos1) / BS);
	do {
		MapNode n = getMap(map).getNode(iterator.m_current_node_pos);
