...
public:
	...
	//
	u16 onserverid = 0; //0 means proxy server
	void SetServerID(u16 ID) { onserverid = ID; }
	void SetServerID(int ID) { onserverid = (uint16_t)ID; }
...
public:
	...
	friend class Server;
	friend class NetworkThread; //This uses the lock
	...
	void sendToAllCompatBySrvID(NetworkPacket *pkt, NetworkPacket *legacypkt, u16 min_proto_ver, u16 ID);
	...
	//TODO find way to avoid this functions; FIXME: May with autolock on each getter func?
