...
void _onGenerated(v3s16 minp, v3s16 maxp, u32 blockseed) {}

_EMERGE_CB_onGenerated_V _EMERGE_CB_onGenerated = &_onGenerated;
...
EmergeManager::EmergeManager(Server *server, HyperMap *MapStruct)
{
	...
	for (s16 i = 0; i < nthreads; i++)
		m_threads.push_back(new EmergeThread(server, i, MapStruct));
...
void EmergeManager::initMapgens(MapgenParams *params)
{
	...
	for (u32 i = 0; i != m_threads.size(); i++) {
		EmergeParams *p = new EmergeParams(this, biomegen, biomemgr, oremgr, decomgr, schemmgr);
		infostream << "EmergeManager: Created params " << p << " for thread " << i << std::endl;
...
EmergeThread::EmergeThread(Server *server, int ethreadid, HyperMap *map) :
	...
	std::map<v3s16, MapBlock *> *modified_blocks)
{
	...
	*/
	/*try {
		m_server->getScriptIface()->environment_OnGenerated(minp, maxp, m_mapgen->blockseed);
	} catch (LuaError &e) {
		m_server->setAsyncFatalError("Lua: finishGen" + std::string(e.what()));
	}*/
	_EMERGE_CB_onGenerated(minp, maxp, m_mapgen->blockseed);
	...
	*/
	...
	/*
		Activate the block
	*/
	m_server->m_env->activateBlock(block, 0, m_map_d->m_id);
