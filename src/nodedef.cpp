...
void NodeDefManager::getAll(std::vector<ContentFeatures> &res) {
	for (ContentFeatures f : m_content_features) {
		res.push_back(f);
	}
}
...
{
	for (content_t id = m_next_id; id >= m_next_id; ++id) {
...
//Edit of the Logiki: I hav edited this to handle raw changes in a specific place of the vector and other things
content_t NodeDefManager::set(const std::string &name, const ContentFeatures &def, content_t forced_id)
{
	// Pre-conditions
	assert(name != "");
	assert(name != "ignore");
	...
	content_t id = CONTENT_IGNORE;
	if (forced_id != 0 || !m_name_id_mapping.getId(name, id)) { // ignore aliases
		if (forced_id != 0) {
			m_name_id_mapping_with_aliases.erase(name);
			//Resize
			if (m_content_features.size() <= forced_id)
				m_content_features.resize(forced_id+1);
		}
		// Get new id
		id = forced_id != 0 ? forced_id : allocateId();
	...
	//if (!m_content_features[id].name.empty())
	//	m_content_features[id] = ContentFeatures();

	verbosestream << "m_content_features size: " << m_content_features.size() 
		<< ", accessing index: " << forced_id << std::endl;

	m_content_features[forced_id != 0 ? forced_id : id] = def;
	verbosestream << "NodeDefManager: registering content id \"" << (forced_id != 0 ? forced_id : id)
...
void NodeDefManager::setMNEXT_ID(u32 new_raw_size) {
	m_next_id = new_raw_size-1;
}
...
void NodeDefManager::deSerialize(std::istream &is)
{
	...
	for (u16 n = 0; n < count; n++) {
		...
		verbosestream << "NodeDef: deserialized " << f.name << std::endl;
