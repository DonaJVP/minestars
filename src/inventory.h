...
//#include "server/serveractiveobject.h" // FOr some reason Kate adds this includes, which makes a complete disaster in compile time.
#include <cstdint>
#include <istream>
#include <ostream>
#include <string>
...
class IInventoryList {
public:
	~IInventoryList() = default;
	virtual void clearItems() = 0;
	virtual void setSize(uint32_t newsize) = 0;
	virtual void setWidth(uint32_t newWidth) = 0;
	virtual void setName(const std::string &name) = 0;
	virtual void serialize(std::ostream &os, bool incremental) const = 0;
	virtual void deSerialize(std::istream &is) = 0;
	virtual const std::string &getName() const = 0;
	virtual uint32_t getSize() const = 0;
	virtual uint32_t getWidth() const = 0;
	virtual uint32_t getUsedSlots() const = 0;
	virtual uint32_t getFreeSlots() const = 0;
	virtual const ItemStack& getItem(uint32_t i) const = 0;
	virtual ItemStack& getItem(uint32_t i) = 0;
	virtual ItemStack changeItem(uint32_t i, const ItemStack &newitem) = 0;
	virtual void deleteItem(uint32_t i) = 0;
	virtual ItemStack addItem(const ItemStack &newitem) = 0;
	virtual ItemStack addItem(uint32_t i, const ItemStack &newitem) = 0;
	virtual bool itemFits(const uint32_t i, const ItemStack &newitem, ItemStack *restitem = NULL) const = 0;
	virtual bool roomForItem(const ItemStack &item) const = 0;
	virtual bool containsItem(const ItemStack &item, bool match_meta) const = 0;
	virtual ItemStack removeItem(const ItemStack &item) = 0;
	virtual ItemStack takeItem(uint32_t i, uint32_t takecount) = 0;
	virtual uint32_t moveItem(uint32_t i, InventoryList *dest, uint32_t dest_i, uint32_t count = 0, bool swap_if_needed = true, bool *did_swap = NULL) = 0;
	virtual void moveItemSomewhere(uint32_t i, InventoryList *dest, uint32_t count) = 0;
};

class InventoryList: public IInventoryList
{
public:
...
using _cualifierReturn_0 = int(*)(const MoveAction&, int, ServerActiveObject*);
using _cualifierReturn_1 = void(*)(const MoveAction&, const ItemStack&, ServerActiveObject*);
using _cualifierReturn_2 = int(*)(const MoveAction&, const ItemStack&, ServerActiveObject*);
using _cualifierReturn_3 = int(*)(const MoveAction&, int, ServerActiveObject*);

// virtual
class IInventory {
public:
	virtual ~IInventory() = default;
	virtual void serialize(std::ostream &os, bool incremental = false) const = 0;
	virtual void deSerialize(std::istream &is) = 0;
	virtual InventoryList *addList(const std::string &name, u32 size) = 0;
	virtual InventoryList *getList(const std::string &name) = 0;
	virtual const InventoryList *getList(const std::string &name) const = 0;
	virtual std::vector<const InventoryList*> getLists() = 0;
	virtual bool deleteList(const std::string &name) = 0;
	virtual ItemStack addItem(const std::string &listname, const ItemStack &newitem) = 0;
};
...
public:
	...
	// Some callbacks.
	_cualifierReturn_0 AllowMove;
	_cualifierReturn_1 onPut;
	_cualifierReturn_1 onTake;
	_cualifierReturn_3 onMove;
	_cualifierReturn_2 AllowPut;
	_cualifierReturn_2 AllowTake;
private:
	// -1 if not found
