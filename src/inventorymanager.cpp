...
bool item_OnCraft__ASSIGN(ItemStack &item, ServerActiveObject *user, const InventoryList *old_craft_grid, const InventoryLocation &craft_inv) {
	return false;
};

_callback__2_IM item_OnCraft = reinterpret_cast<_callback__2_IM>(item_OnCraft__ASSIGN);
_callback__2_IM item_CraftPredict = reinterpret_cast<_callback__2_IM>(item_OnCraft__ASSIGN);
...
//TODO: Maybe change from callbacks to direct inventory action (invObjFirst->callFunc(. . .))

void IMoveAction::onPutAndOnTake(const ItemStack &src_item, ServerActiveObject *player) const
{
	inv_to->onPut(*this, src_item, player);
	inv_from->onTake(*this, src_item, player);
...
{
	inv_from->onMove(*this, count, player);
	/*ServerScripting *sa = PLAYER_TO_SA(player);
	if (from_inv.type == InventoryLocation::DETACHED)
...
{
	return inv_to->AllowPut(*this, dst_item, player);
	/*ServerScripting *sa = PLAYER_TO_SA(player);
...
{
	return inv_from->AllowTake(*this, src_item, player);
	/*ServerScripting *sa = PLAYER_TO_SA(player);
...
{
	// Simple.
	return inv_from->AllowMove(*this, try_take_count, player);
	// ServerScripting moved to addons form.
	/*ServerScripting *sa = PLAYER_TO_SA(player);
	int src_can_take_count = 0xffff;
	if (from_inv.type == InventoryLocation::DETACHED)
...
void IDropAction::apply(InventoryManager *mgr, ServerActiveObject *player, IGameDef *gamedef)
{
	...
	*/
	...
	src_can_take_count = inv_from->AllowTake(*this, src_item, player);
	...
	// Okay..
	if (gamedef->getItemDefManager()->get(item1.name).onDrop(item1, player, player->getBasePosition())) {
	...
	ServerActiveObject *player, IGameDef *gamedef)
{
	...
	*/
	...
	item_CraftPredict(crafted, player, list_craft, craft_inv);
	...
	while (found && list_craftresult->itemFits(0, crafted)) {
		...
		item_OnCraft(crafted, player, &saved_craft_list, craft_inv);
	...
	for (auto &output_replacement : output_replacements) {
		...
		do {
			//PLAYER_TO_SA(player)->item_OnDrop(output_replacement, player, player->getBasePosition());
			gamedef->getItemDefManager()->get(output_replacement.name).onDrop(output_replacement, player, player->getBasePosition());
