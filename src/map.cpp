...
{
	//catch here a exception && MAKE QUEUE!
	if (AlreadyDoingSetNode) {
		warningstream << "Set node skipped" << std::endl;
		return;
	}
	AlreadyDoingSetNode = true;
	v3s16 blockpos = getNodeBlockPos(p);
	MapBlock *block;
	try {
		block = getBlockNoCreate(blockpos);
	} catch (const InvalidPositionException& e) {
		warningstream << "Caught InvalidPositionException: " << e.what() << std::endl;
		AlreadyDoingSetNode = false;
		return;
	}
	v3s16 relpos = p - blockpos*MAP_BLOCKSIZE;
